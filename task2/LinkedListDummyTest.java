...
class LinkedListDummyTest {
    ...
    private static List<Integer> toArray(LinkedListDummy list){
        List<Integer> intList = new ArrayList<>();
        Node item = list.getHead();
        while (item != null) {
            if (!(item instanceof Dummy)) intList.add(item.value);
            item = item.next;
        }
        return intList;
    }

    private static List<Integer> toArrayReverse(LinkedListDummy list) {
        List<Integer> intList = new ArrayList<>();
        Node item = list.getTail();
        while (item != null) {
            if (!(item instanceof Dummy)) intList.add(item.value);
            item = item.prev;
        }
